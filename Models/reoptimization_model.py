...
class ReoptModel:
    ...
    def run_model(self):
        ...
        try:
            ...
            m.addConstrs(
                (q_S[nodes_depots[2 * n + k], k] == L_S[k] - E_K_S[nodes_depots[2 * n + k] for k in vehicles),
                name="SCapacity1",
            )
            ...
            m.addConstrs(
                (q_W[nodes_depots[2 * n + k], k] == L_W[k] - E_K_W[nodes_depots[2 * n + k] for k in vehicles),
                name="WCapacity1",
            )
            m.addConstrs(
                (
