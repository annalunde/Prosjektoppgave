...
try:
    ...
    print("WCapacity1")
    m.addConstrs(
        (q_W[nodes_depots[2 * n + k], k] == 0 for k in vehicles), name="WCapacity1"
    )
    print("WCapacity2")
    m.addConstrs(
        (
    ...
    print("TimeWindow1")
    m.addConstrs(
        (T_S_L[i].timestamp() - l[i] <= t[i, k] for i in nodes for k in vehicles),
        name="TimeWindow1",
    )

    print("TimeWindow2")
    m.addConstrs(
        (t[i, k] <= T_S_U[i].timestamp() + u[i] for i in nodes for k in vehicles),
        name="TimeWindow2",
    )
    ...
    m.addConstrs(
        (
            t[n + i, k] - t[i, k] - (1 + F) * T_ij[i][n + i].total_seconds() <= M * w[i]
            for i in pickups
            for k in vehicles
        ),
        name="RideTime1",
